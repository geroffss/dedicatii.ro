...
export const Categories = () => {
  ...
  const getCategories = async () => {
    ...
    setSelectedCategories(result.data.filter((category) => category.active).map((category) => category.name))
    console.log(result)
  }
      ...
      <div className="w-full p-4">
          ...
          category.active && (
              ...
              >
                <img
                  ...
                  loading="lazy"
              />
                <p className="text-white text-center line-clamp-2 break-words max-w-32">{category.name}</p>
              </button>
          )
        ...
        <div className="flex gap-4">
            {categories?.map((category) => ( 
              <button 
                key={category.id} 
                className={`bg-[#D9D9D9] bg-opacity-10 w-[157px] h-[196px] flex flex-col items-center justify-center cursor-pointer rounded-md ${selectedCategories.includes(category.name) && 'border-2 border-white'}`}
                onClick={() => { 
                  if (!selectedCategories.includes(category.name)) {
                      setSelectedCategories((prevState) => [...prevState, category.name])
                    } else {
                      setSelectedCategories(() => selectedCategories.filter((catName) => catName !== category.name))} 
                }}
              >
                <img
                  src={category.thumbnail}
