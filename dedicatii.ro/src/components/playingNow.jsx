    ...
    return (
      <motion.div {...animationVariants} className='bg-dedicatii-bg2 h-[63rem] w-full absolute top-[56px] left-0 z-10 p-4'>
        ...
        <div className='flex flex-col items-center justify-center w-full h-[456px] bg-[#D9D9D9] bg-opacity-10 rounded-md mt-2'>
          <motion.img {...albumArtAnimation} src={queue.songs[0].thumbnail} alt={queue.songs[0].title} className='h-[264px] w-[260px] mt-2 rounded object-cover' />
          <div className='current-song text-center'>
            <p className="text-[40px] text-white">{queue.songs[0].title}</p>
            <p className='text-[40px] text-white font-light mb-4 mt-2'>{queue.songs[0].artist}</p>
          </div>
        </div>
        ...
        <div className='flex'>
          ...
          <p className="text-xl font-light text-white text-start ml-4 mt-8">Și încă {queue.remaining}</p>
        </div>
